...
public class RequestControllerIntegrationTest {
    ...
    void setup() throws Exception {
        ...
        baseUri = "/users/" + requester.getId() + "/requests";
    ...
    void shouldCreateRequest() throws Exception {
        mockMvc.perform(post(baseUri + "?eventId=" + event.getId())).andExpect(status().isCreated())
                .andExpect(jsonPath("$.event").value(event.getId()))
                .andExpect(jsonPath("$.requester").value(requester.getId()))
                .andExpect(jsonPath("$.status").value("PENDING"));
    }

    @Test
    void shouldReturnRequestList() throws Exception {
        String reqResp = mockMvc.perform(post(baseUri + "?eventId=" + event.getId())).andReturn().getResponse()
                .getContentAsString();
        ParticipationRequestDto req = objectMapper.readValue(reqResp, ParticipationRequestDto.class);

        mockMvc.perform(get(baseUri)).andExpect(status().isOk()).andExpect(jsonPath("$[0].id").value(req.getId()))
                .andExpect(jsonPath("$[0].event").value(event.getId()))
                .andExpect(jsonPath("$[0].requester").value(requester.getId()));
    }

    @Test
    void shouldChangeStateAndReturnRequest() throws Exception {
        String reqResp = mockMvc.perform(post(baseUri + "?eventId=" + event.getId())).andReturn().getResponse()
                .getContentAsString();
        ParticipationRequestDto req = objectMapper.readValue(reqResp, ParticipationRequestDto.class);

        mockMvc.perform(patch(baseUri + "/" + req.getId() + "/cancel")).andExpect(status().isOk())
                .andExpect(jsonPath("$.event").value(event.getId()))
                .andExpect(jsonPath("$.requester").value(requester.getId()))
