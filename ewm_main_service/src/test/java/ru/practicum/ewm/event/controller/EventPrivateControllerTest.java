...
public class EventPrivateControllerTest {
    ...
    void shouldReturnUserEvents() throws Exception {
        ...
        when(eventPrivateService.getUserEvents(1L, 0, 10))
                .thenReturn(list);
    ...
    void shouldReturnEventById() throws Exception {
        when(eventPrivateService.getUserEventById(1L, 2L))
    ...
    void shouldUpdateEvent() throws Exception {
        ...
        when(eventPrivateService.updateUserEvent(eq(1L), eq(2L), any()))
                .thenReturn(new EventFullDto());
    ...
    void shouldAcceptRequest() throws Exception {
        ...
        when(requestPrivateService.acceptRequest(eq(1L), eq(2L), any()))
                .thenReturn(Map.of());
        ...
        verify(requestPrivateService)
                .acceptRequest(eq(1L), eq(2L), any());
    }

    @Test
    void shouldReturnRequests() throws Exception {
        when(requestPrivateService.getEventRequests(1L, 2L))
                .thenReturn(List.of());
        ...
        verify(requestPrivateService)
                .getEventRequests(1L, 2L);
    }

    @Test
    void shouldHandleAccessException() throws Exception {
        when(requestPrivateService.acceptRequest(anyLong(), anyLong(), any()))
                .thenThrow(new AccessRightsException("error"));
